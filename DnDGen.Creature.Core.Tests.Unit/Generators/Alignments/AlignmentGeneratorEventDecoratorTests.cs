...
namespace DnDGen.Creature.Core.Tests.Unit.Generators.Alignments
{
    [TestFixture]
    public class AlignmentGeneratorEventDecoratorTests
    {
        ...
        public void LogEventsForAlignmentGeneration()
        {
            ...
            mockEventQueue.Verify(q => q.Enqueue("DnDGen.Creature.Core", $"Generating alignment for creature name"), Times.Once);
            mockEventQueue.Verify(q => q.Enqueue("DnDGen.Creature.Core", $"Generated lawfulness goodness"), Times.Once);
        }
        ...
        public void LogEventsForNoAlignmentGeneration()
        {
            ...
            mockEventQueue.Verify(q => q.Enqueue(It.IsAny<string>(), It.IsAny<string>()), Times.Exactly(2));
            mockEventQueue.Verify(q => q.Enqueue("DnDGen.Creature.Core", $"Generating alignment for creature name"), Times.Once);
            mockEventQueue.Verify(q => q.Enqueue("DnDGen.Creature.Core", $"Generated no alignment"), Times.Once);
        }
    }
}
